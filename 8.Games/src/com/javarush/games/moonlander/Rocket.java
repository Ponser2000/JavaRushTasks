...
public class Rocket extends GameObject {
  ...
  public Rocket(double x, double y) {
    ...
    List<int[][]> frames = new LinkedList<>();
    downFire = new RocketFire(
        Arrays.asList(ShapeMatrix.FIRE_DOWN_1, ShapeMatrix.FIRE_DOWN_2, ShapeMatrix.FIRE_DOWN_3));
  ...
  public boolean isCollision(GameObject object) {
    ...
    for (int matrixX = 0; matrixX < width; matrixX++) {
      for (int matrixY = 0; matrixY < height; matrixY++) {
        ...
        if (matrix[matrixY][matrixX] != transparent
            && object.matrix[objectY][objectX] != transparent) {
          return true;
        }
      }
    }
